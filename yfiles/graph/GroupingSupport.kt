 ...
 */
external open class GroupingSupport  ( graph: IGraph ) : YObject {
 ...
 * @return The area to enclose for the given group node. This is [Rect.EMPTY] if the node is not a group node, or if it hasn't any children.
 ...
 */
 final   fun calculateMinimumEnclosedArea( groupNode: INode ):Rect
/**
 * Enlarges all group nodes in the graph so that the [minimum enclosed area][calculateMinimumEnclosedArea] is respected.
 ...
 */
 final   fun enlargeAllGroupNodes()
/**
 * Enlarges all group nodes in the graph in an interactive scenario, so that the [minimum enclosed area][calculateMinimumEnclosedArea] is respected.
 ...
 */
 final   fun enlargeAllGroupNodes( context: IInputModeContext )
/**
 * Enlarges the given group node to ensure that the [minimum enclosed][calculateMinimumEnclosedArea] area is inside of its bounds.
 ...
 */
 final   fun enlargeGroupNode( groupNode: INode ,
 ancestors: Boolean )
/**
 ...
 */
 final   fun enlargeGroupNode( context: IInputModeContext ,
 node: INode ,
 ancestors: Boolean )
/**
 ...
 */
 final   fun getDescendants( node: INode ):IEnumerable<INode>
/**
 * Returns all descendants of the provided node that are part of the grouped graph.
 * @param [node] The root node to get the descendants from.
 ...
 */
 final   fun getDescendantsBottomUp( node: INode ):IEnumerable<INode>
/**
...
/**
 * Gets the path to root traversing all [parents][IGraph.getParent] of the given item.
 * @param [node] The node to start.
 * @return A list that includes the node but not the root, unless they are identical and all parents in between them.
 * @see <a href="https://docs.yworks.com/yfileshtml/#/api/GroupingSupport%23GroupingSupport-method-getPathToRoot">Online Documentation</a>
 */
 final   fun getPathToRoot( node: INode ):IList<out INode>
/**
 * Determines whether there are group nodes in the graph.
 * @return `true` if there are [group][IGraph.isGroupNode].
 ...
 */
 final   fun hasGroupNodes():Boolean
/**
 ...
 */
 final   fun isDescendant( node: INode ,
 ancestor: INode ):Boolean
