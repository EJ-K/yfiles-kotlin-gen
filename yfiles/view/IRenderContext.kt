...
external interface IRenderContext : ICanvasContext {
  ...
  /**
   * Gets a transform that can be added to the transform list of a visual to be displayed in the intermediate coordinate system (which still has the same axes directions as the world coordinate system, but doesn't include the [projection][CanvasComponent]).
  ...
  /**
   * Gets a transform that is applied to the contents of the [clip][IRenderContext] rectangle.
   ...
   */
  final fun childVisualRemoved(
    visual: Visual?,
  ): Unit = definedExternally
  
  /**
   * Converts the given set of intermediate coordinates to a coordinate pair that can be used to render within the [viewTransform][IRenderContext].
   ...
   * @param [visual] The visual to be registered. This is the visual which is returned by the [createVisual][IVisualCreator] or [updateVisual][IVisualCreator] method which calls this method.
   * @param [callback] A dispose visual callback which calls [childVisualRemoved][IRenderContext] on all child visuals of the registered `visual`.
   ...
   */
  final fun <TVisual> registerForChildrenIfNecessary(
    visual: TVisual?,
    callback: DisposeVisualCallback<TVisual>,
  ): Unit = definedExternally
   ...
   */
  final fun <TVisual> setDisposeCallback(
    visual: TVisual,
    callback: DisposeVisualCallback<TVisual>,
  ): Unit = definedExternally
  
  /**
   * Converts the given set of world coordinates to a coordinate pair that can be used to render within the [intermediateTransform][IRenderContext].
  ...
  /**
   * Converts the given set of world coordinates to a coordinate pair that can be used to render within the [viewTransform][IRenderContext].
   * @param [worldPoint] The coordinates in the world coordinate system.
   * @return The coordinates in the view coordinate system.
   * @see [viewTransform]
   * @see <a href="https://docs.yworks.com/yfileshtml/#/api/IRenderContext%23IRenderContext-method-worldToViewCoordinates">Online Documentation</a>
   */
