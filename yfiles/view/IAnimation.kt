...
import yfiles.layout.LayoutGraphAdapter
...
external interface IAnimation  {
   ...
   */
  final fun createDelayedAnimation(
    delay: TimeSpan,
  ): IAnimation = definedExternally
   ...
   * @return The newly created [IAnimation].
   ...
   */
  final fun createEasedAnimation(
    easeIn: Double?  = definedExternally,
    easeOut: Double?  = definedExternally,
  ): IAnimation = definedExternally
  ...
  companion object : InterfaceMetadata<IAnimation> {
     ...
     * @param [duration] The value for the [preferredDuration][IAnimation] of the created animation
     * @return An animation that does nothing for the amount of time specified in `duration`.
     ...
     */
    fun createEdgeSegmentAnimation(
      graph: IGraph,
      edge: IEdge,
     ...
     * @param [targetPortLocations] The [IPortLocationModelParameter]s for each [IPort] in the graph that will be animated.
    ...
    fun createGraphAnimation(
      ...
      preferredDuration: TimeSpan?  = definedExternally,
    ): IAnimation
    ...
    /**
     * Creates a new [IAnimation] instance that animates the current graph to the layout given by the [layoutGraph][LayoutGraphAdapter] instance of the given `adapter`.
     * @param [graph] the graph for which the layout should be animated
     * @param [adapter] the [LayoutGraphAdapter] that contains all target layout information
     * @param [preferredDuration] the preferred duration of the animation
     * @return A new [IAnimation] instance that animates the current graph to the layout of the given [layoutGraph][LayoutGraphAdapter].
     ...
     * @param [node] The node whose [layout][INode] to animate.
     ...
     * @param [synchronized] Whether all child animations should be adjusted to have the same (maximum) [preferredDuration][IAnimation]. By default this is enabled.
     ...
     */
    fun createParallelAnimation(
      animations: IEnumerable<IAnimation>,
      synchronized: Boolean  = definedExternally,
    ): IAnimation
     ...
     * @param [preferredDuration] The [preferredDuration][IAnimation] of this animation.
     * @return A new instance of [IAnimation].
     ...
     */
    fun createSequentialAnimation(
      animations: Any /* IEnumerableConvertible<IAnimation> | IEnumerable<WebGLAnimation> */,
    ): IAnimation
     ...
     */
    fun createTableAnimation(
      table: ITable,
      columnLayout: ReadonlyArray<Double>,
      rowLayout: ReadonlyArray<Double>,
    ): IAnimation
    
    /**
     * Creates an animation for the given delegate and [preferredDuration][IAnimation].
     ...
     * @see <a href="https://docs.yworks.com/yfileshtml/#/api/IAnimation%23IAnimation-defaultmethod-fromHandler">Online Documentation</a>
     */
    fun fromHandler(
