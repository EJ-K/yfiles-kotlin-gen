 ...
 */
open external class List<T> () : IList<T>, IListEnumerable<T> {
  /**
   ...
   * @see <a href="https://docs.yworks.com/yfileshtml/#/api/List%23List-constructor-List(IEnumerable,Selector)">Online Documentation</a>
   */ constructor(
   ...
   * @param [match] A predicate function with the signature `(o:Object) -> Boolean` which returns `true` if o matches the conditions of the element to search for.
   ...
   * @param [comparator] The optional comparison function to use. If no comparison function is provided, the elements are sorted by their natural order. Always provide a comparison function for lists containing a mixture of primitive values and other objects. Otherwise, the behavior is undefined.
  ...
  companion object : ClassMetadata<List<*>> {
    ...
    fun <TSource, T> from(
      ...
      mapFunction: Selector<TSource, T>?  = definedExternally,
    ): List<T>
