 ...
 */
external open class EdgeCreator<TDataItem>  () : YObject, IEventDispatcher {

/**
 * Gets or sets an optional provider of an object that will be used as the edge's [IEdge.bends] locations for the source data item.
 * @see [getBends]
...
/**
 * Gets or sets an optional set of bindings that will be [applied][applyStyleBindings] to the edge's [IEdge.style].
...
/**
 * Gets or sets an optional provider of an [IEdgeStyle] instance that will be used as the edge's [IEdge.style] for the source data item.
 * 
 * By default the [defaults]' [IEdgeDefaults.style] will be used. Note that [IEdgeDefaults.shareStyleInstance] will be ignored if this provider returns a value. Use the [styleBindings] to reconfigure properties of the style instances.
 * @see [getStyle]
...
/**
 * Gets or sets an optional provider of an object that will be used as the edge's [ITagOwner.tag][yfiles.graph.ITagOwner.tag] for the source data item.
 * 
 ...
 */
 open   fun createEdge( graph: IGraph ,
 source: INode ,
 target: INode ,
 dataItem: TDataItem ):IEdge
/**
 ...
 */
 open protected   fun createEdgeCore( graph: IGraph ,
 source: INode ,
 target: INode ,
 style: IEdgeStyle ,
 tag: Tag? ):IEdge
 ...
 */
 open protected   fun getBends( dataItem: TDataItem ):IEnumerable<Point>?
/**
 ...
 */
 open protected   fun getStyle( dataItem: TDataItem ):IEdgeStyle?
/**
 ...
 */
 open protected   fun getUpdatedBends( graph: IGraph ,
 edge: IEdge ,
 dataItem: TDataItem ):IEnumerable<Point>?
/**
 ...
 */
 open protected   fun onEdgeCreated( graph: IGraph ,
 edge: IEdge ,
 dataItem: TDataItem )
/**
 ...
 */
 open protected   fun onEdgeUpdated( graph: IGraph ,
 edge: IEdge ,
 dataItem: TDataItem )
/**
 ...
 */
 final   fun updateBends( graph: IGraph ,
 edge: IEdge ,
 dataItem: TDataItem )
/**
 * Updates the edge.
 ...
 */
 open   fun updateEdge( graph: IGraph ,
 edge: IEdge ,
 dataItem: TDataItem )
/**
 ...
 */
 ...
 dataItem: TDataItem )
/**
 * Updates the [ITagOwner.tag][yfiles.graph.ITagOwner.tag] of the `edge` with the given `dataItem` by calling [getUpdatedTag] and setting the new `tag` on the edge.
 * @param [graph] The managed graph.
 * @param [edge] The edge to update.
 * @param [dataItem] The data item that is used for the update.
 ...
 * @see <a href="https://docs.yworks.com/yfileshtml/#/api/EdgeCreator%23EdgeCreated">Online Documentation</a>
 */
fun addEdgeCreatedListener(listener: EventHandler1<GraphBuilderItemEventArgs<IEdge, TDataItem>>)
fun removeEdgeCreatedListener(listener: EventHandler1<GraphBuilderItemEventArgs<IEdge, TDataItem>>)

/**
 * Occurs when a edge has been updated.
 * @see [addEdgeCreatedListener]
 * @see <a href="https://docs.yworks.com/yfileshtml/#/api/EdgeCreator%23EdgeUpdated">Online Documentation</a>
 */
fun addEdgeUpdatedListener(listener: EventHandler1<GraphBuilderItemEventArgs<IEdge, TDataItem>>)
fun removeEdgeUpdatedListener(listener: EventHandler1<GraphBuilderItemEventArgs<IEdge, TDataItem>>)
...
inline fun <TDataItem> EdgeCreator<TDataItem>.addEdgeCreatedHandler(
    ...
    val listener: EventHandler1<GraphBuilderItemEventArgs<IEdge, TDataItem>> = { _, event -> handler(event) }
