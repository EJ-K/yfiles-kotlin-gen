...
external interface IParseContext : ILookup {
  ...
  /**
   * Returns the most current (the last element) within the container hierarchy as it is returned by [objectStack][IParseContext] or `default(T)` if the most current element is not of the desired target type.
   * @param [T] The desired target type.
   ...
   */
  fun <T : Any> getDeserializationProperty(
