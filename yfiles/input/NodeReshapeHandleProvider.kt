 ...
 * 
 * @constructor Creates a default instance that provides handles for each of the `handlePositions` given for the `node`.
 ...
 */
open external class NodeReshapeHandleProvider (
...
handlePositions: HandlePositions) : ReshapeHandleProviderBase {
  ...
  /**
   * Gets or sets an [INodeSizeConstraintProvider] that is passed to the [queried][NodeReshapeHandleProvider] handles.
  ...
  /**
   * Provides a [NodeReshapeHandlerHandle] that uses the [reshapeHandler][NodeReshapeHandleProvider] to perform the actual reshaping.
